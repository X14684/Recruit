<template>
  <div class="college-students-page">
    ...
    <div class="search-bar">
      <div class="search-input-wrap">
        ...
        <input v-model="keyword" placeholder="搜索学生姓名/学号..." @keyup.enter="fetchList" />
      </div>
      ...
      <el-table :data="list" style="width:100%">
        ...
        <el-table-column label="姓名" prop="realName" width="100">
          <template #default="{ row }">
            <div class="name-cell">{{ row.realName }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="求职意向" prop="intentionPosition" min-width="130" />
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await getStudentList({ collegeId: userStore.userInfo?.id, page: page.value, size: 10, keyword: keyword.value })
...
<style scoped>
.college-students-page{padding-bottom:40px}
...
.search-input-wrap{
  ...
  background:#fff;border:1px solid rgba(226,232,240,.85);flex:1;max-width:320px;
  transition:all .25s ease
}
.search-input-wrap:focus-within{border-color:rgba(59,130,246,.4);box-shadow:0 0 0 3px rgba(59,130,246,.06)}
.search-input-wrap .el-icon{color:#94A3B8;font-size:16px;flex-shrink:0}
.search-input-wrap input{border:none;outline:none;background:transparent;font-size:13.5px;color:#334155;width:100%;letter-spacing:.005em}
...
.name-cell{font-weight:600;color:#1E293B;display:inline-flex;align-items:center;gap:8px}
.status-dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px}.status-dot.green{background:#22C55E;box-shadow:0 0 5px rgba(34,197,94,.4)}.status-dot.red{background:#EF4444;box-shadow:0 0 5px rgba(239,68,68,.4)}

.pagination-wrap{display:flex;justify-content:center;margin-top:20px}
@media(max-width:768px){.search-bar{flex-direction:column}.search-input-wrap{max-width:none}}
</style>